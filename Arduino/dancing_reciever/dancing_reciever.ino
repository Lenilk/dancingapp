...
#define sw5 D5
#define sw6 D6
const int swGrp[] = { sw1, sw2, sw3, sw4, sw5, sw6 };
int value;
...
void readSwitchValue() {
  ...
  if (amount == 0) {
    ispass = true;
    Serial.println("PASS NO PAD");
    return;
  }
  if (amount == 1) {
    if (digitalRead(swGrp[currentNote.padTouch[0] - 1])==1) {
      score += 1;
      ispass = true;
      Serial.print("PASS: ");
      Serial.println(currentNote.padTouch[0]);
    }
  } else {
    for (i = 0; i < 2; i++) {
      key = swGrp[currentNote.padTouch[i] - 1];
      if (key != thispass) {
        val = digitalRead(key);
        if (val == 1) {
          Serial.print("PASS: ");
          Serial.println(currentNote.padTouch[i]);
          if (thispass != -1) {
            ispass = true;
            score += 1;
            break;
...
void checkAmountOfPadTouch() {
  ...
  amount = k;
  Serial.print(F("Amount of Pad Touch "));
  Serial.println(k);
}
